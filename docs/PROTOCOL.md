...
| `B\|A` | **Switch account** — continue the shown conversation on whichever account has the most plan limit left. The device deliberately does not choose: ranking needs every profile's usage, which lives on the Mac, and a four-button device is the wrong place to browse logins. The daemon sends `switch --best` to the wrapper, which owns the claude process and so is the only thing that can replace it — it copies the transcript into the target profile and re-execs itself with `--resume`. |
