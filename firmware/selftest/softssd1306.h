...
class SoftSSD1306 : public Adafruit_GFX {
 public:
  ...
  // Send one bare command byte (e.g. 0xA7/0xA6 display invert for the press
  // blip). MUST NOT be called while flushBusy(): the chunked flush holds the
  // I2C data transaction open between pumpFlush() calls, and a command needs
  // its own transaction. Callers poll flushBusy() first.
