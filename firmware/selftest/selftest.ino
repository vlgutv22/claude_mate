...
// Active-tab rectangle: r3 is "N/5 E B D W I" -> letters at col 4 + i*2 (space
// between). Demo i's own letter sits at col 4 + i*2. Odd demos show FOLLOW ►.
...
static void drawDemo(uint8_t i) {
  ...
  // Active-tab selection rectangle centred on this demo's own fleet letter
  // (col 4 + i*2): a wide filled block (lit rectangle, letter knocked out).
  // Odd demos also show the FOLLOW-mode "play" triangle by the state row.
  int16_t bx = (int16_t)(4 + i * 2) * 6 - 3;
  if (bx < 0) bx = 0;
  display.fillRect(bx, 24, 11, 8, SSD1306_INVERSE);
