...
That row existed for exactly one reason: a factory-reset board with no cable had
no way to be given a token. [BLE enrolment](#enrolment) removed that reason —
`claude-mate-connect --pair` puts **PAIR?** on the glass and one press of GO
provisions the device over the link it was already using. What was left was a
Wi-Fi flow wearing a label that said *Set token*.

The order here matters and is not taste: removing this row **before** pairing
existed stranded a real board within minutes, and `tools/test_ble_link.py`
asserts the `E|?` handler exists in the same breath as asserting the row does
not. The portal is still reachable where a mistake cannot reach it — `Z` over
USB, or **BOOT** (or **GO**) held at power-on — because a board whose BLE will
not start needs something, and both of those need physical access.
...
The S3 has **one 2.4 GHz radio**, shared between Wi-Fi and BLE, so holding both
up costs latency on the link that matters — and Wi-Fi is the half that was
misbehaving. But a device that goes deaf for a whole session is worse than a
laggy one: the daemon is the reason this thing exists, and an error you hear
about an hour later is an error you heard about too late.

So the radio follows your thumbs, because nothing else can tell it. **A HID
gamepad is one-way** — the browser has no channel back to the device — so "are
you mid-level?" is answered by "did you press anything recently?".

| State | Radio | Why |
|---|---|---|
| **Pressing things** (within `PAD_PLAY_IDLE_MS`, 6 s) | Wi-Fi parked via `net.shutdown()` | You are playing. `poll()` no-ops in `OFF`, so nothing blocks the loop and BLE gets the air to itself |
| **Still** for 6 s | Wi-Fi back up, link re-dialled | You stopped to read a card, or paused, or wandered off. Go and see what the daemon has |
| **Urgent alert arrives** (`V\|ERROR` or `V\|INPUT`) | Leaves the pad entirely, back to CONDUCTOR | Those two mean a human is blocked. `DONE` and `START` are news, and news can wait until you stop |

The radio is never parked mid-attempt: once it comes up it stays up for at least
...
| `x no token - claude-mate-connect` | `THE DEVICE HAS NO TOKEN` | exactly what it says, and it is what a factory-reset board says. Run `claude-mate-connect --pair` and press **GO** on the device. With a cable, plugging it in is enough — the daemon provisions it on port open |
